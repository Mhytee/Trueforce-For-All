...
Assetto Corsa does not publish its finished steering force anywhere the plugin can read. Custom Shaders Patch has one place where that number exists: the FFB post-processing slot, a hook CSP offers for scripts that want to shape the force on its way to the wheel.
...
It does not change your games force feedback settings, in the game or in CSP, and removing it puts Assetto Corsa back exactly as it was.
