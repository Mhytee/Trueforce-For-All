iRacing keeps working out what the car is doing. It just stops driving the wheel itself and hands those forces to the plugin, so the feel stays the sim's own, and your rev lights and wheel screen come back as well.

The plugin starts on **Lightsync only** in iRacing, which leaves the sim completely alone. On a wheel whose lights can take a pattern, it also sets that pattern to match the car you are in. For the force feedback itself, four steps, once:
