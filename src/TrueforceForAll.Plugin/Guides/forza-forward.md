...
Because Forza only sends to one place, pointing it at the plugin means SimHub stops receiving it. Dashboards, ShakeIt bass shakers, anything SimHub drives from Forza telemetry goes quiet.
...
Leave forwarding off if you do not use SimHub for anything in Forza. It costs nothing either way, but there is no reason to put packets on the network that nothing is listening for.
