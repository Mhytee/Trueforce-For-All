...
It also frees your wheel's rev lights and screen. Those share a channel with the game's force feedback, so they can only run when the plugin owns the whole signal, which is exactly what this does. [Why that is](guide:wheel-lights).
...
- Farming Simulator 22 and 25, where the plugin replaces the game's force feedback with its own entirely. It engages by itself, with nothing to switch on and no mod required. [The telemetry mod](guide:farming-sim) is a separate thing: it adds ground texture, the implement thud and the airborne cut on top of the takeover.
...
> [!WARNING]
> Two force streams fighting each other feels jumpy and buzzy. The Telemetry FFB tab warns you when it detects that, but it is easier to avoid than to diagnose.
...
Turn iRacing's force feedback **off** instead, and leave its strength where it is. The [iRacing setup guide](guide:iracing-setup) walks through both switches.
