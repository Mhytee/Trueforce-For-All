...
//
// The colors we send are sRGB: #FFFF00 means yellow because that is what
...
//
// Nothing else in the pipeline touches a color byte, by design: patterns are
...
// insight behind how this works: if a color lights only one channel there is
...
//
// How much comes back depends on WHICH channel is the color's peak, and this
...
// means a color peaked on it renormalises by exactly 1 and keeps the full cut
...
// brightness BETWEEN colors, since the green die out-emits the red one at
...
// the user's own colors on every restore, walking them toward black.
//
    ...
    public static class LedColorGain
    {
        ...
        public static byte[] Apply(byte[] rgb, float gR, float gG, float gB)
        {
            ...
            for (int i = 0; i + 2 < rgb.Length; i += 3)
            {
                ...
                // strip is ten independent colors, and letting one bright LED
        ...
        /// pure color anyway. Split this per chassis the moment anyone reports
