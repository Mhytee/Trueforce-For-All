...
; TF4ALL Dash (DashStudio dashboard, generated by dashboards\make-tf4all-dash.ps1).
#define DashboardsDir  "..\dashboards"
...
; TF4ALL Dash -> SimHub's dashboard library, so it appears in Dash Studio and
; the web dash picker with no import step. Overwritten on upgrade so dash
; updates ship with the plugin; a user's own edited COPY (different folder
; name) is untouched. SimHub identifies dashboards by folder, so this adds
; exactly one entry.
Source: "{#DashboardsDir}\TF4ALL Dash\*"; DestDir: "{app}\DashTemplates\TF4ALL Dash"; Flags: ignoreversion recursesubdirs createallsubdirs
