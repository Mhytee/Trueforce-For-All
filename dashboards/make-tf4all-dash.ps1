...
function DriveBox([string]$P, [int]$slot, $x, $y, $w, $h, [bool]$topRow) {
    ...
    $t = New-Text "d$slot-cf-mf-t" ($ix + 10) ($iy + 160) ($iw - 20) 30 13 '' $script:GRAY 0 @{
        Text = BindJS 'Text' (
            'var rdy=$prop("' + $P + '.IRacingAutoReady");var n=1*$prop("' + $P + '.IRacingAutoNm");var u=$prop("' + $P + '.IRacingAutoUnit");' +
            'if(rdy)return "SET PEAK FORCE  "+(n>0?n.toFixed(1):"--")+(u?(" "+u):"");' +
...
function IdleCard([string]$P) {
    ...
    for ($ei = 0; $ei -lt $EPOOL; $ei++) {
        $eu  = $ei / [double]$EPOOL
        $eci = $ei % 3
        $eki = [int][math]::Floor($ei / 3)      # 0..13, ring index
        $egc = $ei % 7                          # grid column
        $egr = [int][math]::Floor($ei / 7)      # grid row, 0..5
...
$mf = New-Text 'cf-mf-value' 32 402 300 28 20 '' $WHITE 0 @{
    Text = BindJS 'Text' ('var v=1*$prop("' + $P + '.IRacingMaxForceNm");var u=$prop("' + $P + '.IRacingAutoUnit");return v>0?(v.toFixed(1)+(u?(" "+u):"")):"--"')
} 'Bold'
$mf.Bindings['Visible'] = BindJS 'Visible' $mfVis; $s2.Add($mf)
...
$mf = New-Text 'cf-mf-t' 552 388 216 38 15 '' $GRAY 1 @{
    Text = BindJS 'Text' (
        'var rdy=$prop("' + $P + '.IRacingAutoReady");var n=1*$prop("' + $P + '.IRacingAutoNm");var u=$prop("' + $P + '.IRacingAutoUnit");' +
        'return rdy&&n>0?("AUTO  "+n.toFixed(1)+(u?(" "+u):"")):"AUTO"')
