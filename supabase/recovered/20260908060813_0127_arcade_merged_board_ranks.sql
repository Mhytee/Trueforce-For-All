-- Per-board ranks across the merged field, in ONE place.
--
-- get_arcade_scores_merged already computed these and threw them away, keeping only the total. The
-- player readout wants them too: "18 first times among TF4ALL users" says nothing without the
-- worldwide number beside it, which is the gap the owner spotted. Rather than a second copy of the
-- ranking, both callers read this. Two copies of a ranking formula is the bug 0120 existed to fix.
