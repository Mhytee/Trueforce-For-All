...
begin
    ...
    if p_fact_type = 'engine_layout' then
        ...
        -- Gear 0 (the overall/default redline) is represented by 'rpm' for
        -- back-compat, so only forward gears 1..16 live in the array. Canonical
        -- form: validated, deduped by gear (lowest rpm wins), sorted ascending,
        -- so identical per-gear profiles hash identically and cluster in consensus.
        v_gears := null;
        if p_payload ? 'gears'
