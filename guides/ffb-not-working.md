...
That points at [USBPcap](usbpcap.md). It captures the game's force feedback off the USB bus. When it is not capturing, the game's force never reaches the wheel and only our effects do.
...
## The wheel is limp and [Telemetry Based FFB](telemetry-ffb.md) is on for this game
...
1. Check the mode at the top of the panel is **Normal**, not Off or Lightsync only. The plugin drops to Lightsync only by itself when a game streams its own Trueforce beside it; a popup and the amber line under the status say so, and [Games with native Trueforce](native-trueforce.md) has the cure.
...
If the wheel responds but everything feels faint, that is a different problem: see [Effects feel weak](weak-effects.md).
