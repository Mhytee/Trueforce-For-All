...
The TF4ALL CSP Bridge is a small script that runs inside Assetto Corsa and hands the game's force feedback to the plugin directly. [Installing it](assetto-corsa-setup.md) takes one click.
...
- **The Dynamic OLED and LED pattern changes stop costing force.** With the plugin carrying the force, the wheel's FFB does not cut out when we write to the LEDs or Dynamic OLED. [Why](force-and-lights.md).
