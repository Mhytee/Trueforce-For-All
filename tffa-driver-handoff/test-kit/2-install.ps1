...
if (-not (Test-Path $inf)) { throw "TFFAUsbFilter.inf not found in $Dir - copy the whole kit folder, don't just copy the scripts." }
...
Write-Host ""
Write-Host "Installed. Now REBOOT so the USB filter attaches." -ForegroundColor Green
Write-Host "After reboot:" -ForegroundColor Cyan
Write-Host "  1. Run DebugView.exe as admin -> Capture menu -> 'Capture Kernel' + 'Enable Verbose Kernel Output'."
Write-Host "  2. Plug in the wheel. Watch for 'TFFAUsbFilter:' lines."
Write-Host "     - 'DeviceAdd MATCH (...) ffbIdx=0xNN' = filter attached and configured for your wheel."
Write-Host "     - Wheel still enumerates in Device Manager = safe."
