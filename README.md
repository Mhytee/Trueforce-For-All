...
Rev lights and the OLED screen need [Telemetry Based FFB](#telemetry-based-ffb)
switched on. They share a control channel with the wheel's force, so they can
only run when the plugin is generating that force itself.
...
Four pairs of controls can be bound to anything SimHub can see, a wheel
button, a button box, a keyboard key, so the things worth changing
